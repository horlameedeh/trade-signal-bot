...
def _cleanup_guarded_live_data(db) -> None:
    ...
    db.execute(text("DELETE FROM users WHERE display_name LIKE 'guard-user-%'"))
    db.commit()
    ...
    *,
    ...
    db_session.execute(
        ...
        ),
        {
            ...
            "display_name": f"guard-user-{account_id}",
        },
    )
    ...
    db_session.execute(
        text(
            ...
            VALUES (
              CAST(:account_id AS uuid), CAST(:user_id AS uuid), :broker, 'mt5', 'personal_live', 'guard-seed',
                            ARRAY[]::provider_code[], :equity_start, :equity_start, false
            )
            """
        ),
        {"account_id": account_id, "user_id": user_id, "broker": broker, "equity_start": equity_start},
    )
