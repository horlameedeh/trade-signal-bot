...
def _cleanup_heartbeat_terminal_data() -> None:
    with SessionLocal() as db:
        db.execute(
            text(
                "DELETE FROM terminal_sessions WHERE terminal_name LIKE 'heartbeat-terminal-%'"
            )
        )
        db.execute(
            text(
...
def _cleanup_heartbeat_data():
    _cleanup_heartbeat_terminal_data()
    yield
    _cleanup_heartbeat_terminal_data()
