    ...
    while True:
        try:
            ...
            for upd in data.get("result", []):
                ...
                onboarding_cmd = handle_onboarding_command(
                    text=text_msg,
                    telegram_user_id=uid,
                    display_name=None,
                )

                if onboarding_cmd.handled:
                    tg_post(
                        cfg,
                        "sendMessage",
                        {
                            "chat_id": control_chat_id,
                            "text": onboarding_cmd.message,
                        },
                    )
                    continue

                account_cmd = handle_account_command(
                    text=text_msg,
                    telegram_user_id=uid,
                )

                if account_cmd.handled:
                    tg_post(
                        cfg,
                        "sendMessage",
                        {
                            "chat_id": control_chat_id,
                            "text": account_cmd.message,
                        },
                    )
                    continue
