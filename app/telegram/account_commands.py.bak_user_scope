...
def handle_account_command(*, text: str, telegram_user_id: int | None) -> AccountCommandResult:
    ...
    try:
        if command == "!addaccount":
            ...
            label = parts[1].strip()
            upsert_broker_credentials(BrokerCredentialInput(account_label=label))
        ...
        if command == "!showaccount":
            if len(parts) < 2:
                return AccountCommandResult(True, False, _usage())
            label = parts[1].strip()
            return AccountCommandResult(True, True, safe_show_account(label))
        ...
        if command == "!setbroker":
            upsert_broker_credentials(BrokerCredentialInput(account_label=label, broker=value.lower()))
        ...
        if command == "!setmt":
            ...
            upsert_broker_credentials(BrokerCredentialInput(account_label=label, platform=platform))
            return AccountCommandResult(True, True, f"✅ Platform updated for {label}: {platform}")

        if command == "!setlogin":
            upsert_broker_credentials(BrokerCredentialInput(account_label=label, login=value))
            return AccountCommandResult(True, True, f"✅ Login updated for {label}")

        if command == "!setpassword":
            upsert_broker_credentials(BrokerCredentialInput(account_label=label, password=value))
            return AccountCommandResult(True, True, f"✅ Password updated for {label} (hidden)")

        if command == "!setserver":
            upsert_broker_credentials(BrokerCredentialInput(account_label=label, server=value))
